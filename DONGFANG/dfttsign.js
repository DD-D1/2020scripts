...
最新版没有 如果你用的版本有那就把211行到216行禁用取消
=========================================
...
if (!dftoutiaoheaderArr[0]) {
    ...
    return;
  } else {console.log(`\n************ 脚本共${dftoutiaoheaderArr.length}个${$.name}账号  ************\n`
  );
  console.log(`\n============ 脚本执行时间(TM)：${new Date(new Date().getTime() + 0 * 60 * 60 * 1000).toLocaleString('zh', {hour12: false})}  =============\n`)}

  for (let i = 0; i < dftoutiaoheaderArr.length; i++) {
if(dftoutiaoheaderArr[i]){
